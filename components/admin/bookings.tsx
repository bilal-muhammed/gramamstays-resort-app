        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1 overflow-x-auto">
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-colors whitespace-nowrap ${activeFilter === f ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              {f}
            </button>
          ))}
        </div>
          ...
          <tbody>
            {filtered.map(b => (
              <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50">
                <td className="px-4 py-3 text-xs font-semibold text-gray-900">{b.id}</td>
                <td className="px-4 py-3">
                  <p className="text-sm font-medium text-gray-900">{b.guest}</p>
                  <p className="text-[10px] text-gray-400">{b.email}</p>
                </td>
                <td className="px-4 py-3 text-sm text-gray-600">{b.room} #{b.roomNo}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{b.checkIn} - {b.checkOut}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-sm font-bold text-gray-900 text-right">${b.amount.toLocaleString()}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => openEdit(b)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600"><Pencil size={13} /></button>
                    <button onClick={() => setDeleteConfirm(b.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
          ...
          <div key={b.id} className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                <button onClick={() => openEdit(b)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400"><Pencil size={13} /></button>
                <button onClick={() => setDeleteConfirm(b.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={13} /></button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg sm:mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2 pb-1">
                ...
                <button type="submit" className="flex-1 py-3 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary/90">{editingId ? 'Update' : 'Create'} Booking</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full sm:max-w-sm sm:mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(deleteConfirm)} className="flex-1 py-3 rounded-lg bg-red-500 text-white text-sm font-semibold hover:bg-red-600">Delete</button>
            </div>
          </div>
        </div>
      )}
